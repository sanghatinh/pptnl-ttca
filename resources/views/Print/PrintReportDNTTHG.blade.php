...
@if(in_array('phieugiaonhan', $reportData['selected_sections']))
    <div class="section-header">
        <i>📋</i> Phiếu giao nhận Hom Giông
    </div>
    
    @if(isset($reportData['sections_data']['phieugiaonhan']) && count($reportData['sections_data']['phieugiaonhan']) > 0)
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Tổng thực nhận (Tấn)</th>
                        <th>Tổng tiền (KIP)</th>
                    </tr>
                </thead>
                <tbody>
                    @php $totalThucNhan = 0; $totalAmount = 0; @endphp
                    @foreach($reportData['sections_data']['phieugiaonhan'] as $index => $item)
                        @php
                            $totalThucNhan += $item->tong_thuc_nhan ?? 0;
                        ...
                        <tr>
                            ...
                            <td>{{ $item->ten_phieu ?? 'N/A' }}</td>
                            <td>{{ $item->hop_dong_dau_tu_mia_ben_giao_hom ?? 'N/A' }}</td>
                            <td>{{ $item->khach_hang_ca_nhan ?? 'N/A' }}</td>
                            <td>{{ $item->khach_hang_doanh_nghiep ?? 'N/A' }}</td>
                            <td class="text-right">{{ number_format($item->tong_thuc_nhan ?? 0, 2, ',', '.') }}</td>
                ...
                <tfoot>
                    <tr>
                        <td colspan="8" class="text-center"><strong>TỔNG CỘNG</strong></td>
                        <td class="text-right"><strong>{{ number_format($totalThucNhan, 2, ',', '.') }}</strong></td>
    ...
    @else
        <div class="empty-section">
            <i>📄</i>
            <p>Không có dữ liệu phiếu giao nhận Hom Giong</p>
        </div>
    @endif
@endif

@if(in_array('chitietgiaonhan', $reportData['selected_sections']))
    <div class="section-header">
        <i>📝</i> Chi tiết giao nhận Hom Giông
    </div>
    
    @if(isset($reportData['sections_data']['chitietgiaonhan']) && count($reportData['sections_data']['chitietgiaonhan']) > 0)
        <div class="table-container">
           <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    @php $totalThucNhan = 0; $totalThanhTien = 0; @endphp
                    @foreach($reportData['sections_data']['chitietgiaonhan'] as $index => $item)
                        @php
                            $totalThucNhan += $item->thuc_nhan ?? 0;
                            $totalThanhTien += $item->thanh_tien_hom_giong ?? 0;
                        @endphp
                        <tr>
                            <td class="text-center">{{ $index + 1 }}</td>
                            <td>{{ $item->ma_so_phieu ?? 'N/A' }}</td>
                            <td>{{ $item->tram ?? 'N/A' }}</td>
                            <td>{{ $item->phieu_gn_hom_giong ?? 'N/A' }}</td>
                            <td>{{ $item->giong_mia ?? 'N/A' }}</td>
                            <td>{{ $item->thua_dat ?? 'N/A' }}</td>
                            <td>{{ $item->don_vi_tinh ?? 'N/A' }}</td>
                            <td class="text-right">{{ number_format($item->thuc_nhan ?? 0, 2, ',', '.') }}</td>
                            <td class="text-right">{{ number_format($item->don_gia ?? 0, 0, ',', '.') }}</td>
                            <td class="text-right amount">{{ number_format($item->thanh_tien_hom_giong ?? 0, 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="7" class="text-center"><strong>TỔNG CỘNG</strong></td>
                        <td class="text-right"><strong>{{ number_format($totalThucNhan, 2, ',', '.') }}</strong></td>
                        <td colspan="1"></td>
                        <td class="text-right amount"><strong>{{ number_format($totalThanhTien, 0, ',', '.') }}</strong></td>
                    </tr>
                </tfoot>
            </table>
        </div>
    @else
        <div class="empty-section">
            <i>📝</i>
            <p>Không có dữ liệu chi tiết giao nhận Hom Giong</p>
        </div>
    @endif
@endif
        ...
        <div class="mt-3" style="border-top: 1px solid #dee2e6; padding-top: 15px; font-size: 10px; color: #6c757d;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    ...
                    @php
                        $sectionNames = [
                    'phieugiaonhan' => 'Phiếu giao nhận Hom Giông',
                    'chitietgiaonhan' => 'Chi tiết giao nhận Hom Giông'
