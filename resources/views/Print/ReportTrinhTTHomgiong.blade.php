...
<head>
    ...
    <title>Báo cáo Phiếu trình thanh toán - TTC Attapeu</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="title">
                <h1>PHIẾU GIAO NHẬN HỒ SƠ THANH TOÁN HOM GIỐNG</h1>
                <p>Ngày tạo: {{ $reportInfo['generated_date'] ?? now()->format('d/m/Y H:i:s') }}</p>
                <p>Tổng số bản ghi: {{ $reportInfo['total_records'] ?? 0 }}</p>
                <p>Loại báo cáo: {{ $reportInfo['report_type_display'] ?? 'Phiếu trình thanh toán' }}</p>
            </div>
        </div>
            ...
            <div class="info-grid">
                <div class="info-section">
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value">{{ $reportInfo['title'] ?? 'Báo cáo Phiếu trình thanh toán' }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Số tờ trình:</span>
                        <span class="info-value">{{ $reportInfo['so_to_trinh'] ?? 'N/A' }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Số đợt thanh toán:</span>
                        <span class="info-value">{{ $reportInfo['so_dot_thanh_toan'] ?? 'N/A' }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Loại thanh toán:</span>
                        <span class="info-value">{{ $reportInfo['loai_thanh_toan'] ?? 'N/A' }}</span>
                    </div>
                    <div class="info-row">
                ...
                <div class="info-section">
                    ...
                    @php
                    $totalThucNhan = collect($reportData)->sum('thuc_nhan') ?? 0;  // Add this line
                        $totalPaid = collect($reportData)->sum('da_tra_goc');
                        $totalInterest = collect($reportData)->sum('tien_lai');
                        $totalDeduction = collect($reportData)->sum('tong_tien_khau_tru') ?? 0;
                        $totalHoldAmount = collect($reportData)->sum('tong_tien_tam_giu') ?? 0;
                        $totalRemaining = collect($reportData)->sum('tong_tien_thanh_toan_con_lai') ?? 0;
                    @endphp
                     <div class="info-row">
        <span class="info-label">Tổng thực nhận:</span>
        <span class="info-value amount">{{ ($totalThucNhan) }} TẤN</span>
    </div>
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value amount">{{ number_format($totalDeduction) }} KIP</span>
                    </div>
        ...
        @if(count($reportData) > 0)
            <div class="section-header">
                <i class="fas fa-table"></i> Danh sách chi tiết phiếu trình thanh toán
            </div>
    ...
    <thead>
        <tr>
            ...
            <th>Khách hàng</th>
            <th>Mã khách hàng</th>
            <th class="text-right">Thực nhận (TẤN)</th>  
            <th class="text-right">Tổng tiền (KIP)</th>
            <!-- New column for thuc_nhan -->
            <th class="text-right">Tạm giữ (KIP)</th>
            <th class="text-right">Khấu trừ (KIP)</th>
            <th class="text-right">Tiền lãi (KIP)</th>
            <th class="text-right">Còn lại (KIP)</th>
        </tr>
    </thead>
    <tbody>
        @foreach($reportData as $index => $item)
            <tr>
                ...
                <td class="text-right amount">{{ $item['thuc_nhan'] ?? 0 }}</td>  <!-- Display thuc_nhan -->
                <td class="text-right amount">{{ number_format($item['da_tra_goc']) }}</td>
                <td class="text-right amount">{{ number_format($item['tong_tien_tam_giu'] ?? 0) }}</td>
                <td class="text-right amount">{{ number_format($item['tong_tien_khau_tru'] ?? 0) }}</td>
                <td class="text-right amount">{{ number_format($item['tien_lai'] ?? 0) }}</td>
                <td class="text-right amount">{{ number_format($item['tong_tien_thanh_toan_con_lai'] ?? 0) }}</td>
            </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr>
            ...
            <td class="text-right amount">
                <strong>{{ collect($reportData)->sum('thuc_nhan') ?? 0 }}</strong>  <!-- Sum of thuc_nhan -->
            </td>
            <td class="text-right amount">
            ...
            <td class="text-right amount">
                <strong>{{ number_format(collect($reportData)->sum('tong_tien_tam_giu') ?? 0) }}</strong>
            </td>
            <td class="text-right amount">
                <strong>{{ number_format(collect($reportData)->sum('tong_tien_khau_tru') ?? 0) }}</strong>
            </td>
            <td class="text-right amount">
                <strong>{{ number_format(collect($reportData)->sum('tien_lai') ?? 0) }}</strong>
            </td>
            <td class="text-right amount">
                <strong>{{ number_format(collect($reportData)->sum('tong_tien_thanh_toan_con_lai') ?? 0) }}</strong>
            </td>
        </tr>
    </tfoot>
</table>
        ...
        @else
            <div class="empty-section">
                ...
                <p>Không tìm thấy phiếu trình thanh toán nào phù hợp với tiêu chí lọc</p>
            </div>
        @endif
