...
<head>
    ...
    <title>Báo cáo Công nợ dịch vụ phải thu - TTC Attapeu</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="title">
                <h1>BÁO CÁO CÔNG NỢ DỊCH VỤ PHẢI THU</h1>
        ...
        <div class="document-info">
            <div class="info-grid">
                ...
                <div class="info-section">
                    ...
                    @endphp
                    <div class="info-row">
                        <span class="info-label">Tổng nợ gốc:</span>
                        <span class="info-value amount">{{ number_format($totalDebt) }} KIP</span>
                    </div>
                    <div class="info-row">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-right amount">
                                <strong>{{ number_format(collect($reportData)->sum('tong_no_goc')) }}</strong>
                            </td>
                            <td class="text-right amount">
                                <strong>{{ number_format(collect($reportData)->sum('da_tra_goc')) }}</strong>
                            </td>
                            <td class="text-right amount">
                                <strong>{{ number_format(collect($reportData)->sum('so_tien_con_lai')) }}</strong>
                            </td>
                            <td class="text-right amount">
                    ...
                    <tbody>
                        @php
                            $typesSummary = collect($reportData)->groupBy('loai_dau_tu')->map(function($group) {
                                return [
                                    'count' => $group->count(),
                                    'total_debt' => $group->sum('tong_no_goc'),
                                    'total_paid' => $group->sum('da_tra_goc'),
                                    'total_remaining' => $group->sum('so_tien_con_lai'),
                        ...
                        @foreach($typesSummary as $type => $summary)
                            <tr>
                                ...
                                <td class="text-right amount">{{ number_format($summary['total_debt']) }}</td>
                                <td class="text-right amount">{{ number_format($summary['total_paid']) }}</td>
                                <td class="text-right amount">{{ number_format($summary['total_remaining']) }}</td>
