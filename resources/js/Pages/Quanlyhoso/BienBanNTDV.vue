    ...
    >
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header bg-light">
                    <h5
                        class="modal-title text-info"
                        id="importChiTietModalLabel"
                    >
                        <i class="fas fa-upload text-info me-2"></i>
                        Import Chi tiết NT
                    </h5>
                ...
                <div class="modal-body">
                    <div class="alert alert-warning mb-3">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        <small>
                            <strong>Cảnh báo:</strong> Import chi tiết NT sẽ xóa
                            <u>tất cả</u> dữ liệu hiện có trong bảng chi tiết và
                            thay thế bằng dữ liệu mới từ file. Vui lòng xác minh
                            thông tin là chính xác trước khi tiếp tục.
                        </small>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="importChiTietFile" class="form-label"
                            >Select file</label
                        >
                        <input
                            class="form-control"
                    ...
                    <div v-if="chiTietProcessingRecords" class="mb-3">
                        ...
                        <div class="progress">
                            <div
                                ...
                                role="progressbar"
                                :style="{
                                    width: chiTietProcessingProgress + '%',
                                }"
                                :aria-valuenow="chiTietProcessingProgress"
                                aria-valuemin="0"
                                aria-valuemax="100"
                            >
                                {{ chiTietProcessedRecords }}/{{
                                    chiTietTotalRecords
                                }}
                            </div>
                        </div>
    ...
    methods: {
            ...
            try {
                import("bootstrap/dist/js/bootstrap.bundle.min.js")
                    .then((bootstrap) => {
                        ...
                        if (modalElement) {
                            this.reportModal = new Modal(modalElement);
        ...
        async generateReportCurrentPage() {
            if (this.paginatedItems.data.length === 0) {
                Swal.fire({
                    ...
                    text: "Không có dữ liệu nào để tạo báo cáo",
                    icon: "warning",
        ...
        async generateReport(reportData, reportType) {
            try {
                ...
                const filterParams = {
                    ma_nghiem_thu_list: reportData,
                    search: this.search,
                    status_filter: this.statusFilter,
                    investment_filter: this.investmentFilter,
                    delivery_status_filter: this.deliveryStatusFilter,
                ...
                const result = await Swal.fire({
                    ...
                    text: `Bạn có muốn tạo báo cáo PDF cho ${reportData.length} bản ghi không?`,
                    icon: "question",
                ...
                queryParams.append("token", token); // เพิ่ม token เป็น query parameter

                // Create URL for opening in new tab
                const reportUrl = `/api/generate-report-bienban-ntdv?${queryParams.toString()}`;
                ...
                Swal.fire({
                    title: "Thành công!",
                    text: "Đã mở trang báo cáo trong tab mới",
                    icon: "success",
                    timer: 3000,
                    ...
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            ...
                            Swal.fire({
                                title: "Thành công",
                                text:
                                    `Đã tải lên thành công ${data.processed} bản ghi` +
        ...
        async checkImportChiTietProgress(importId) {
            if (!importId) {
                this.chiTietImportErrors = [
                    "Invalid import ID. Please try again.",
                ];
                return;
            }
                    ...
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            ...
                            Swal.fire({
                                title: "Thành công",
                                text:
                                    `Đã tải lên thành công ${data.processed} bản ghi Chi tiết NT` +
    ...
    mounted() {
        ...
        import("bootstrap/dist/js/bootstrap.bundle.min.js")
            .then((bootstrap) => {
                ...
                const importChiTietModalElement =
                    document.getElementById("importChiTietModal");
                if (importChiTietModalElement) {
                    this.importChiTietModal = new bootstrap.Modal(
                        importChiTietModalElement
                    );
                }
            })
            .catch((err) => {
