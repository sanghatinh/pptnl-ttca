                                    ...
                                    >
                                        ...
                                        <tbody>
                                            ...
                                            >
                                                ...
                                                <td>
                                                    <router-link
                                                        :to="`/Details_PhieuGiaoNhanHomGiong/${item.ma_so_phieu}`"
                                                        class="receipt-link"
                                                    >
                                                        {{ item.ma_so_phieu }}
                                                    </router-link>
                                ...
                                <div class="table-responsive mt-2">
                                    <table
                                        ...
                                        v-resizable-columns="{
                                            minWidth: 80,
                                            saveState: true,
                                            id: 'dntt-chitiet-giaonhan-table',
                                            adjustTableWidth: false,
                                        }"
                                        style="table-layout: fixed"
                                    >
                                        ...
                                        <tbody>
                                            ...
                                            >
                                                ...
                                                <td>
                                                    {{
                                                        item.phieu_gn_hom_giong
                                                    }}
                                                </td>
                                                <td>{{ item.giong_mia }}</td>
                                                <td>{{ item.thua_dat }}</td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td class="text-end fw-bold">
                                                    {{
                                                        parseFloat(
                                                            chitietTotals.total_thanh_tien ||
                                                                0
                                                        ).toLocaleString(
                                                            "en-US",
                                                            {
                        ...
                        <div class="col-12">
                            ...
                            <div class="form-check mb-3">
                                <input
                                    ...
                                    type="checkbox"
                                    value="phieugiaonhan"
                                    id="printPhieuGiaoNhan"
                                    v-model="printOptions.sections"
                                />
                                <label
                                    class="form-check-label"
                                    for="printPhieuGiaoNhan"
                                >
                                    <div class="d-flex align-items-center">
                                        <i
                            ...
                            <div class="form-check mb-3">
                                <input
                                    class="form-check-input"
                                    type="checkbox"
                                    value="chitietgiaonhan"
                                    id="printChiTietGiaoNhan"
                                    v-model="printOptions.sections"
                                />
                                <label
                                    class="form-check-label"
                                    for="printChiTietGiaoNhan"
                                >
                                    <div class="d-flex align-items-center">
                                        <i
...
export default {
    ...
    data() {
        return {
            ...
            totals: {
                total_thuc_nhan: 0, // เปลี่ยนจาก total_payment_amount
                total_amount: 0, // เปลี่ยนจาก total_amount
            },
            chitietGiaoNhanHG: [], // Array to hold chi tiết nghiệm thu dịch vụ data
            chitietTotals: {
                // เพิ่มตัวแปรสำหรับ totals
                total_thuc_nhan: 0,
                total_thanh_tien: 0,
            },
    ...
    mounted() {
        this.fetchUserData();
        this.fetchDocument();
        this.fetchgiaonhanHG();
        this.fetchChitietgiaonhanHG();
    ...
    methods: {
        ...
        async executePrintReport() {
            ...
            try {
                ...
                if (!paymentRequestId) {
                    throw new Error(
                        "Không tìm thấy mã phiếu đề nghị thanh toán"
                    );
                }
        ...
        async fetchInvestmentProject(maVuDauTu) {
            if (!maVuDauTu) return "";
            ...
            try {
                ...
                // ลองใช้ GET กับ query parameters แทน
                const response = await axios.get("/api/investment-projects", {
                    params: {
                        ma_vu_dau_tu: maVuDauTu,
                    },
                    headers: {
                ...
                if (response.data && response.data.success) {
                    // หา project ที่ตรงกับ Ma_Vudautu
                    const projects = response.data.data || [];
                    const project = projects.find(
                        (p) => p.Ma_Vudautu === maVuDauTu
                    );

                    if (project) {
                        return project.Ten_Vudautu || maVuDauTu;
                    }
                }

                // ถ้าไม่เจอหรือ API ไม่ success ให้ return ค่าเดิม
                return maVuDauTu;
            } catch (error) {
                console.error("Error fetching investment project:", error);
                console.error("Error response:", error.response?.data);

                // ถ้าเกิด error ให้ return ค่าเดิม
                return maVuDauTu;
            }
        },
        async fetchChitietgiaonhanHG() {
            try {
                // เปลี่ยนจาก this.documentId เป็น this.$route.params.id
                ...
                const response = await axios.get(
                    `/api/payment-requests-homgiong/${id}/chitiet-giaonhan`,
                    {
                        headers: {
                            Authorization: `Bearer ${this.store.getToken}`,
                        },
                ...
                if (response.data && response.data.success) {
                    this.chitietGiaoNhanHG = response.data.data || [];
                    this.chitietTotals = response.data.totals || {
                        total_thuc_nhan: 0,
                        total_thanh_tien: 0,
                    };
                } else {
                    this.chitietGiaoNhanHG = [];
                    this.chitietTotals = {
                        total_thuc_nhan: 0,
                        total_thanh_tien: 0,
                    };
                ...
                console.error(
                    "Error fetching chitiet giaonhan homgiong:",
                    error
                );
                this.showError(
                    "Lỗi khi tải dữ liệu chi tiết giao nhận hom giống"
                );
                this.chitietGiaoNhanHG = [];
                this.chitietTotals = {
                    total_thuc_nhan: 0,
        ...
        fetchgiaonhanHG() {
            ...
            axios
                .get(`/api/payment-requests-homgiong/${id}/phieu-giao-nhan`, {
                    headers: {
                        Authorization: "Bearer " + this.store.getToken,
                    },
                })
                    ...
                    if (response.data.success) {
                        ...
                        this.showError(
                            response.data.message ||
                                "Không thể tải dữ liệu phiếu giao nhận hom giống"
                        );
                    }
                })
                .catch((error) => {
                    console.error(
                        "Error fetching phieu giao nhan hom giong data:",
                        error
                    );
                    this.showError(
                        "Lỗi khi tải dữ liệu phiếu giao nhận hom giống"
                    );
                    if (error.response?.status === 401) {
                        this.handleAuthError();
                    }
        ...
        async fetchDocument() {
            ...
            try {
                ...
                if (response.data.success) {
                    this.document = {
                        ma_giai_ngan: response.data.document.ma_giai_ngan || "",
                        ...
                        khach_hang_doanh_nghiep:
                            response.data.document.khach_hang_doanh_nghiep ||
                            "",
                        ma_khach_hang_doanh_nghiep:
                            response.data.document.ma_khach_hang_doanh_nghiep ||
                            "",
                        ...
                        so_to_trinh: response.data.document.so_to_trinh || "",
                        so_dot_thanh_toan:
                            response.data.document.so_dot_thanh_toan || "",
                        comment: response.data.document.comment || "",
                    };

                    // Fetch investment project name
                    if (this.document.vu_dau_tu) {
                        console.log(
                            "Fetching project for:",
                            this.document.vu_dau_tu
                        ); // เพิ่มบรรทัดนี้
                        this.investmentProjectName =
                            await this.fetchInvestmentProject(
                                this.document.vu_dau_tu
                            );
                    }
        ...
        async fetchProcessingHistory() {
            ...
            try {
                const response = await axios.get(
                    `/api/payment-requests-homgiong/${id}/processing-history`,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                            "Content-Type": "application/json",
        ...
        updateFormFields() {
            ...
            if (vuDauTu)
                vuDauTu.value =
                    this.investmentProjectName || this.document.vu_dau_tu || "";
        ...
        UpdateDocument() {
            ...
            axios
                .put(
                    `/api/payment-requests-homgiong/${id}/update`,
                    updateData,
                    {
                        headers: {
                            Authorization: "Bearer " + this.store.getToken,
